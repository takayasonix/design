...
export async function generateStaticParams() {
  ...
  try {
    ...
    // log.jsonからarticleカテゴリのアイテムを取得してslugを生成
    const articles = logData.filter((item) => item.category === 'article');
...
async function getMetadataFromLogJson(slug: string): Promise<LogItem | null> {
  const filePath = path.join(process.cwd(), 'public', 'api', 'log.json');
  ...
  try {
    ...
    const article = logData.find((item) => 
      item.category === 'article' && item.date.replace(/\//g, '') === slug
    );
...
async function getMarkdownContent(slug: string, category: 'article'): Promise<{ content: string; metadata: LogItem | null }> {
  try {
    const username = process.env.GITHUB_LOG_USERNAME || 'takayasonix';
    const repositoryName = process.env.GITHUB_LOG_REPOSITORY || 'brain';
    const path = category === 'article' ? 'content/blog' : 'content/log';
        ...
        if (dateSectionMatch) {
          ...
          if (startMatch) {
            ...
            if (fileSlug === slug) {
              ...
              // タイトルを取得（blogの場合は「名前」を優先）
              let titleSectionMatch = frontMatter.match(/名前:\s*\n([\s\S]*?)(?=\n\w+:|$)/);
...
export default async function BlogPage({ params }: PageProps) {
  ...
  // log.jsonからメタデータを取得（画像パスが正しく設定されている）
  const metadataFromJson = await getMetadataFromLogJson(slug);
  const { content, metadata: metadataFromMarkdown } = await getMarkdownContent(slug, 'article');

  // log.jsonから取得したメタデータを使用（画像パスが正しい）
  const metadata = metadataFromJson || metadataFromMarkdown;
